...
/**
 * Processes all four case-type-gating templates independently, so if one
 * succeeds and another fails, every result is still reported clearly —
 * never lets one failure abort processing of the rest.
 */
...
export async function run(): Promise<void> {
  try {
    ...
    console.error("✗ Failed to create case-type Content Templates");
    console.error(message);
    process.exitCode = 1;
  }
}

// Only auto-run when executed directly (`tsx create-case-type-templates.ts` /
// `npm run twilio:case-type:create`) — not when imported by tests.
