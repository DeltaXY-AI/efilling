...
export async function run(): Promise<void> {
  try {
    ...
    console.error("✗ Failed to verify filing Content Templates");
    console.error(message);
    process.exitCode = 1;
  }
}

// Only auto-run when executed directly (`tsx verify-filing-templates.ts` /
// `npm run twilio:filing:verify`) — not when imported by tests.
