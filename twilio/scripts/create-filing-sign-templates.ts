...
interface FilingSignTemplateEntry {
  label: string;
  fileName: string;
  envVar: string;
}

/** #34 (Prototype parity — Phase 6)'s only rich Content Template — the draft-ready "Review & e-Sign / Edit details" quick-reply. The OTP prompt/error have no Content Template (plain text only — see filing-sign-workflow.ts). */
const FILING_SIGN_TEMPLATES: FilingSignTemplateEntry[] = [
  { label: "Filing draft-ready actions (English)", fileName: "filing-draft-ready-actions.en.json", envVar: "TWILIO_FILING_DRAFT_READY_ACTIONS_SID_EN" },
  { label: "Filing draft-ready actions (Malayalam)", fileName: "filing-draft-ready-actions.ml.json", envVar: "TWILIO_FILING_DRAFT_READY_ACTIONS_SID_ML" },
];
...
/**
 * Processes both #34 templates independently, so if one succeeds and the
 * other fails, every result is still reported clearly (mirrors
 * create-filing-details-templates.ts).
 */
export async function main(): Promise<void> {
  const credentials = loadTwilioCredentialsFromEnv();
  let anyFailed = false;

  for (const entry of FILING_SIGN_TEMPLATES) {
...
export async function run(): Promise<void> {
  try {
    ...
    console.error("✗ Failed to create filing-sign Content Templates");
    console.error(message);
    process.exitCode = 1;
  }
}

// Only auto-run when executed directly (`tsx create-filing-sign-templates.ts` /
// `npm run twilio:filing-sign:create`) — not when imported by tests.
