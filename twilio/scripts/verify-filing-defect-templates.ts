...
/** Mirrors create-filing-defect-templates.ts's list exactly. */
...
/**
 * Verifies all 10 configured Content SIDs independently, reporting each
 * result clearly (mirrors verify-filing-draft-list-templates.ts).
 */
...
export async function run(): Promise<void> {
  try {
    ...
    console.error("✗ Failed to verify filing-defect Content Templates");
    console.error(message);
    process.exitCode = 1;
  }
}

// Only auto-run when executed directly (`tsx verify-filing-defect-templates.ts` /
// `npm run twilio:filing-defect:verify`) — not when imported by tests.
