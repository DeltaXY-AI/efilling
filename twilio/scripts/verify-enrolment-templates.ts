...
export async function run(): Promise<void> {
  try {
    ...
    console.error("✗ Failed to verify advocate-enrolment Content Templates");
    console.error(message);
    process.exitCode = 1;
  }
}

// Only auto-run when executed directly (`tsx verify-enrolment-templates.ts` /
// `npm run twilio:enrolment:verify`) — not when imported by tests.
