...
/** Mirrors create-filing-completion-templates.ts's list exactly. */
 ...
 * result clearly (mirrors verify-filing-sign-templates.ts).
 */
...
export async function run(): Promise<void> {
  try {
    ...
    console.error("✗ Failed to verify filing-completion Content Templates");
    console.error(message);
    process.exitCode = 1;
  }
}

// Only auto-run when executed directly (`tsx verify-filing-completion-templates.ts` /
// `npm run twilio:filing-completion:verify`) — not when imported by tests.
