...
interface FilingTemplateEntry {
  label: string;
  fileName: string;
  envVar: string;
}

const FILING_TEMPLATES: FilingTemplateEntry[] = [
  { label: "Draft choice (English)", fileName: "filing-draft-choice.en.json", envVar: "TWILIO_FILING_DRAFT_CHOICE_SID_EN" },
  { label: "Draft choice (Malayalam)", fileName: "filing-draft-choice.ml.json", envVar: "TWILIO_FILING_DRAFT_CHOICE_SID_ML" },
  { label: "Test notice (English)", fileName: "filing-notice.en.json", envVar: "TWILIO_FILING_NOTICE_SID_EN" },
  { label: "Test notice (Malayalam)", fileName: "filing-notice.ml.json", envVar: "TWILIO_FILING_NOTICE_SID_ML" },
];
...
/**
 * Processes all four V5A templates independently, so if one succeeds and
...
export async function main(): Promise<void> {
  ...
  for (const entry of FILING_TEMPLATES) {
...
export async function run(): Promise<void> {
  try {
    ...
    console.error("✗ Failed to create filing Content Templates");
    console.error(message);
    process.exitCode = 1;
  }
}

// Only auto-run when executed directly (`tsx create-filing-templates.ts` /
// `npm run twilio:filing:create`) — not when imported by tests.
