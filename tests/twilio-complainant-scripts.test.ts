...
describe("Twilio complainant-details Content Template scripts", () => {
  ...
  describe("verify-complainant-templates", () => {
    beforeEach(() => {
      process.env.TWILIO_COMPLAINANT_REVIEW_SID_EN = "HXconfiguredA00000000000000000000";
      process.env.TWILIO_COMPLAINANT_REVIEW_SID_ML = "HXconfiguredB00000000000000000000";
      process.env.TWILIO_COMPLAINANT_EDIT_FIELDS_SID_EN = "HXconfiguredC00000000000000000000";
      process.env.TWILIO_COMPLAINANT_EDIT_FIELDS_SID_ML = "HXconfiguredD00000000000000000000";
    });

    it("succeeds when all four configured SIDs match their specifications", async () => {
      const sids = [
        process.env.TWILIO_COMPLAINANT_REVIEW_SID_EN!,
        process.env.TWILIO_COMPLAINANT_REVIEW_SID_ML!,
        process.env.TWILIO_COMPLAINANT_EDIT_FIELDS_SID_EN!,
        process.env.TWILIO_COMPLAINANT_EDIT_FIELDS_SID_ML!,
      ];
    ...
    it("reports a missing SID for one template while still verifying the rest", async () => {
      delete process.env.TWILIO_COMPLAINANT_EDIT_FIELDS_SID_ML;
      const sids = [
        process.env.TWILIO_COMPLAINANT_REVIEW_SID_EN!,
        process.env.TWILIO_COMPLAINANT_REVIEW_SID_ML!,
        process.env.TWILIO_COMPLAINANT_EDIT_FIELDS_SID_EN!,
      ...
      expect(loggedOutput()).toContain("TWILIO_COMPLAINANT_EDIT_FIELDS_SID_ML is not configured");
    });

    it("reports a mismatch against the remote template without aborting the rest", async () => {
      const mismatched = resourceFrom(SPECS[0], process.env.TWILIO_COMPLAINANT_REVIEW_SID_EN!, {
